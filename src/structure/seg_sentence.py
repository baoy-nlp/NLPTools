...
class SegSentence(object):
    ...
    def fwd_bigram(self):
        ...
        for i in range(self.n - 1):
            self._fwd_bigram.append(self.seg_sentence[i][0] + self.seg_sentence[i + 1][0])
        self._fwd_bigram = [('SS')] + self._fwd_bigram + [('/S/S')]
        return self._fwd_bigram

    def unigram(self):
        self._unigram = []
        for i in range(self.n):
            self._unigram.append(self.seg_sentence[i][0])
    ...
    def __str__(self):
        ...
        for i in range(1, self.n0 + 1):
            if self.seg_sentence[i][1] == 'NA':
                str += (' ' + self.seg_sentence[i][0])
            elif self.seg_sentence[i][1] == 'AP':
                str += self.seg_sentence[i][0]
    ...
    def get_spans(self):
        ...
        while right <= self.n0 + 1:
            if self.seg_sentence[right][1] == 'NA':
                result.add((left, right - 1))
                left = right
                right += 1
            elif self.seg_sentence[right][1] == 'AP':
                right += 1
...
class Sentence(SegSentence):
    def __init__(self,sentence):
        seg = [(c,l) for (c,_,l) in sentence]
        pos = [(c,t) for (c,t,_) in sentence]
        super(Sentence, self).__init__(seg)
        self.pos_sentence = PosSentence(pos)
    ...
    def compare(self, other):
        fscore = SegSentence.compare(self,other)
        accuracy = Accuracy()
...
class PosSentence(object):
    def __init__(self,sentence):
        self.st_tuples = self.span_tag_tuple(sentence) ## :: [(left,right,tag)]
        self.sentence = self.ct2wt(sentence)  ##  :: [(Word,Tag)] with Sentence.START and Sentence.END
    ...
    @staticmethod
    def span_tag_tuple(sentence):
        st_tuple = []
        left = 1
        right = 1
        tag = sentence[1][1]
        for (i,(c,t)) in enumerate(sentence[1:-1]):
            if t == 'NotStart':
                right += 1
            else:
                st_tuple.append((left,right,tag))
                tag = t
                left = i+1
                right = left
        return st_tuple
    ...
    def span_tags(self,i,j):
        for left,right,tag in self.st_tuples:
